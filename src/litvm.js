...
function run(schema, data, startRule, opt) {
  ...
  // Начальное X дельта-кодека. Для блоков дерева POI его надо засеять НОМЕРОМ
  // БЛОКА: ссылки на потомков — это позиции (номер блока, ключ записи), и
  // кодируются они относительно текущего блока. С засевом все 3322 ссылки
  // блока 101099 попадают в блоки с узлами или тайлами, без засева мимо летят
  // 1475; то же на семи других блоках дерева, везде 100 % против ~52 %.
  //
  // По умолчанию 0 — универсальность засева НЕ доказана: на обычном блоке POI
  // он меняет значения в 1159 записях из 1946 (границы блока при этом те же,
  // на число прочитанных байт он не влияет вовсе). Чем X инициализируется в
  // прошивке на самом деле, пока не найдено.
  let X = o.x0 || 0, Y = 0, baseX = o.x0 || 0;
