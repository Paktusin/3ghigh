...
function run(schema, data, startRule, opt) {
  ...
  // Начальное X дельта-кодека — это НОМЕР БЛОКА, и это прочитано в коде, а не
  // подобрано по данным. Цепочка такая: `FUN_08cceb48` («перейти в позицию»)
  // отдаёт номер блока в `FUN_08cc78a8`, та кладёт его в поле объекта +0xc0 и
  // передаёт дальше, а `FUN_08cd070c` (создание состояния разбора) копирует
  // ОТТУДА ШЕСТЬ БАЙТ в `состояние+0xc`:
  //
  //     (*(code *)PTR_FUN_08cd078c)(param_1 + 3, param_5, 6);   // memcpy
  //
  // `состояние+0xc` — это и есть база X дельта-кодека (`param_1[3]` в
  // `FUN_08cd0798`: формы «база» и «база + байт» читают именно её), а следующие
  // два байта — база Y; их инициализирует `FUN_08ccf670` нулём. База X внутри
  // машины только читается, ни одна ветка её не пишет.
  //
  // Шесть байт — это позиция (номер блока u32, ключ записи u16), тем же
  // размером её копирует и обход дерева (`memcpy(&local_5c, &local_54, 6)`).
  // Отсюда и совпадение с замером: с засевом все 3322 ссылки блока 101099 и
  // все 26 106 ссылок восьми блоков дерева попадают в блоки с узлами или
  // тайлами, без засева мимо летит около половины.
  //
  // Текущее X (`param_1[5]`) при открытии блока НЕ сбрасывается — остаётся от
  // предыдущего; здесь оно ставится в базу, потому что первое чтение в блоке
  // всегда абсолютное или базовое.
  const blk = o.blk !== undefined ? o.blk : (o.x0 || 0);
  let X = blk, Y = 0, baseX = blk;
  let mark = 0, home = -1, lastHome = -1, byGoto = false;
