...
function relocate(outDir, codes, anchor, toLon, toLat, log, noRas, zero, noNachbarn) {
  ...
  // контрольная сумма (+0x20), которую движок сверяет при чтении. Без
  // пересчёта она устаревает у каждого тронутого файла. Пересчитываем по
  // актуальным данным: тайлы в контейнере XAC3, а также .xah и .ras в XAC.
  // (Отдельная поблочная сумма CIFF — другой уровень, здесь не покрывается.)
  const touched = [];
