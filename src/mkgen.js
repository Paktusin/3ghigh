...
// пересобранного блока сверяются с исходными, и расхождение прекращает сборку.
//
...
//
// Чего набор не проверяет: межблочные векторы при пересборке теряются (их
// номер указывает в таблицу соседнего блока, а наша нумерация своя). Маршрут
// внутри блока считается, через границу блоков — нет. Для вопроса «читает ли
// движок наши байты» этого хватает, для готовой карты — нет.
//
...
function mkgen(outDir, codes, log) {
  ...
  for (const { c, codes: list } of byContainer.values()) {
    ...
    for (const code of list) {
      ...
      const { out, stat } = xg.regenTile(buf);
      ...
      log('   межблочных векторов потеряно ' + stat.cross +
