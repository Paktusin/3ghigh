...
//   0x66  позиция (номер блока, ключ записи) — ссылка в область названий
...
function poisOf(schema, block, blk, opt) {
  ...
  for (const rec of r.records) {
    ...
    // и ссылка ведёт В ТОТ ЖЕ блок (X у неё равен его номеру, ключи идут
    // подряд). Что именно адресует ключ — не выяснено, поэтому короткие
    // записи отдаются как есть, с пометкой `short`.
    const poi = {
