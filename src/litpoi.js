...
function poisOf(schema, block, blk, opt) {
  ...
  for (const rec of r.records) {
    ...
    // Форм у записи две, и различает их ВЕДУЩИЙ БАЙТ: старшие пять бит — тип
    // (0xa8 у точки), младшие три — флаги. Бит 2 включает всю полезную часть
    // (категория, название, подробности); без него остаются только безусловные
    // поля — ссылка 0x66, координата и идентификатор. Это и есть «короткая
    // форма», она помечена `short`. Сверено на 1180 записях, исключений нет.
    //
    // Ключ в ссылке — порядковый номер элемента в блоке, ноль у первого
    // элемента после заголовка (рамки потомков сошлись у 16 886 пар, 0
    // расхождений). В блоках с повторяющимися группами нумерация идёт и по
    // виткам — там ключ пока не воспроизводится.
    const poi = {
